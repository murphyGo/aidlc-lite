...
This is the **aidlc-lite template repo itself**, not a project using it. After cloning, the user runs `/lite-init` and this file is replaced with a project-specific AGENTS.md.
...
| Skill | Purpose |
|-------|---------|
| `/lite-init` | Idea capture entry point — auto-detects greenfield / brownfield / refinement, generates or updates BRIEF, PLAN, DECISIONS, project AGENTS.md |
...
| `/lite-team` | Optional. Analyzes the project and proposes an agent team (team-lead + signal-driven specialists). User approves; skill writes `.agents/agents/*.md` + `TEAM.md` |
...
| `.agents/skills/lite-init/SKILL.md` | Init skill definition |
| `.agents/skills/lite-init/templates/BRIEF.md` | Template for the project's one-page brief |
| `.agents/skills/lite-init/templates/PLAN.md` | Template for the project's flat checklist |
| `.agents/skills/lite-init/templates/DECISIONS.md` | Template for the project's ADR log |
| `.agents/skills/lite-dev/SKILL.md` | Dev driver skill definition |
| `.agents/skills/code-review/SKILL.md` | Code review skill (trimmed from aidlc-starter) |
| `.agents/skills/code-review/protocols/` | 7 deep-analysis protocols + INDEX (loaded on demand) |
| `.agents/skills/lite-team/SKILL.md` | Meta-skill: proposes and generates an agent team based on project signals |
| `.agents/skills/lite-team/templates/agent-base.md` | Skeleton for each generated agent (frontmatter + AIDLC awareness + out-of-bounds) |
| `.agents/skills/lite-team/templates/TEAM.md` | Template for the project's team manifest (written to project root on team gen) |
